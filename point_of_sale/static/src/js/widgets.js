    ...
    module.OrderWidget = module.PosBaseWidget.extend({
        ...
        init: function(parent, options) {
            ...
            this.line_dblclick_handler = function(event){
                ...
                self.pos.get('selectedOrder').selectLine(this.orderline);
                var ok = false;
                if ($.inArray(self.pos.get('selectedOrder').selected_orderline.product.default_code,['00A', '00B', '00C', 'ç1', 'ç2', 'ç3']) != -1){
            	    do {
            	    	var producto = prompt ("Introduzca nuevo nombre de producto");
            	    	if (producto == null) {		//Cancelar
            	    		producto = self.pos.get('selectedOrder').selected_orderline.product.display_name;
            	    		ok = true;
            	    	}
            	    	else if (producto.length > 0){
            	    		self.pos.get('selectedOrder').selected_orderline.product.display_name = producto;
            	    		ok = true;
            	    	}
            	    	else {
            	    		alert ("Nombre de producto vacío.\nPor favor, establezca algo.");
            	    	}
            	    } while (!ok);
                }
                self.pos_widget.numpad.state.reset();
