...
class ProductProductLabel(models.TransientModel):
    ...
    def get_filename(self):
        return 'etiquetas.xls'

    def get_content(self):
        global record
        res = StringIO()
        wb = xlwt.Workbook() 
        sheet = wb.add_sheet('Etiquetas')
        sheet.write(0, 0, 'Codigo EAN')
        sheet.write(0, 1, 'Codigo')
        sheet.write(0, 2, 'Descripcion')
        sheet.write(0, 3, 'Atributos')
        row = 1
        for product in self.browse([record]).label_lines:
            for i in range (product.quantity):
                sheet.write (row, 0, str (product.product_id.ean13 or ''))
                sheet.write (row, 1, (product.product_id.default_code or ''))
                sheet.write (row, 2, product.product_id.name)
                att = ''
                for at in product.product_id.attribute_value_ids:
                    att += at.name
                    att += ' '
                sheet.write (row, 3, att)
                row += 1
        wb.save(res)
        res.seek(0)
        return res.read()
