...
class sale_order_line(osv.osv):
    ...
    def _cent_margin(self, cr, uid, ids, field_name, arg, context=None):
        cur_obj = self.pool.get('res.currency')
        res = {}
        for line in self.browse(cr, uid, ids, context=context):
            cur = line.order_id.pricelist_id.currency_id
            res[line.id] = 0
            if line.product_id and line.purchase_price or line.product_id.standard_price:
                tmp_margin = ((line.price_subtotal - ((line.purchase_price or line.product_id.standard_price) * line.product_uos_qty))*100) /  ((line.purchase_price or line.product_id.standard_price) * line.product_uos_qty)
                res[line.id] = cur_obj.round(cr, uid, cur, tmp_margin)
        return res
    ...
    _columns = {
        'margin': fields.function(_product_margin, string='Margin', digits_compute= dp.get_precision('Product Price'),
              store = True),
        'cent_margin': fields.function(_cent_margin, string='% Margen', digits_compute= dp.get_precision('Product Price'),
              store = True),
...
class sale_order(osv.osv):
    ...
    def _cent_margin(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for sale in self.browse(cr, uid, ids, context=context):
            ...
            for line in sale.order_line:
                if line.state == 'cancel':
                    continue
                marg += line.margin or 0.0
                cost += (line.purchase_price * line.product_uos_qty) or 0.0
                if cost > 0:
                    result[sale.id] = (marg * 100) / cost
        return result
    ...
    _columns = {
        ...
        'cent_margin': fields.function(_cent_margin, string='% Margen', store={
                'sale.order.line': (_get_order, ['margin', 'purchase_price'], 20),
                'sale.order': (lambda self, cr, uid, ids, c={}: ids, ['order_line'], 20),
                }, digits_compute= dp.get_precision('Product Price')),
    }
