...
class ProjectTaskMaterials(models.Model):
    ...
    def _prepare_analytic_line(self):
        ...
        company_id = self.env['res.company']._company_default_get(
            'account.analytic.line')
        journal = self.pool['account.analytic.journal'].search(self._cr, 1, [('company_id', '=', company_id),('name', '=', 'Tasks Materials')])[0] #self.env.ref('project_task_materials_stock.analytic_journal_sale_materials') 
        ...
        amount_dic = analytic_line_obj.on_change_unit_amount(
            ...
            company_id, False, journal, self._context)    #journal.id
